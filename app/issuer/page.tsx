...
export default function IssuerPortalPage() {
  ...
  useEffect(() => {
    ...
    if (params.get("mode") === "reset") {
      setMode("reset");
      // Arriving here from an email link means /auth/callback already exchanged
      // the code for a recovery session, so the new-password form can be shown.
  ...
  return <main className="mx-auto max-w-xl px-4 py-10 sm:px-6"><section className="rounded-2xl border border-slate-200 bg-white p-6 shadow-sm dark:border-slate-700 dark:bg-slate-900 sm:p-8"><div className="mb-6 text-center"><div className="mx-auto flex h-12 w-12 items-center justify-center rounded-2xl bg-slate-900 text-cyan-300"><ShieldCheck className="h-6 w-6" /></div><h1 className="mt-4 text-3xl font-bold text-slate-900 dark:text-white">Issuer access</h1><p className="mt-2 text-sm text-slate-600 dark:text-slate-300">Register or sign in to manage your certificates.</p></div>
    ...
    {mode === "reset" ? <form className="space-y-4" onSubmit={submit}><label className="block text-sm font-medium text-slate-700 dark:text-slate-200">Registered email<input required type="email" value={email} onChange={(event) => setEmail(event.target.value)} className="input-field mt-1" /></label><button disabled={loading} className="btn-primary w-full">{loading ? "Sending..." : "Send reset instructions"}</button>{recoverySession && <div className="rounded-lg border border-amber-200 bg-amber-50 p-3 text-xs text-amber-800">Development reset link available. Set a new password below.</div>}</form> : <form className="space-y-4" onSubmit={submit}>{mode === "register" ? <>{(["issuerName", "slug", "organizationName", "displayName"] as const).map((field) => <label key={field} className="block text-sm font-medium text-slate-700 dark:text-slate-200">{field === "issuerName" ? "Issuer name" : field === "organizationName" ? "Organization name" : field === "displayName" ? "Your name" : "Issuer slug"}<input required value={registration[field]} onChange={(event) => setRegistration({ ...registration, [field]: event.target.value })} className="input-field mt-1" /></label>)}<label className="block text-sm font-medium text-slate-700 dark:text-slate-200">Email<input required type="email" value={registration.email} onChange={(event) => setRegistration({ ...registration, email: event.target.value })} className="input-field mt-1" /></label><label className="block text-sm font-medium text-slate-700 dark:text-slate-200">Password<input required type="password" minLength={10} pattern="(?=.*[A-Z])(?=.*[a-z])(?=.*[0-9]).{10,}" title="Use at least 10 characters, including uppercase, lowercase, and a number." value={registration.password} onChange={(event) => setRegistration({ ...registration, password: event.target.value })} className="input-field mt-1" /><span className="mt-1 block text-xs text-slate-500">Minimum 10 characters with uppercase, lowercase, and number.</span></label></> : <><label className="block text-sm font-medium text-slate-700 dark:text-slate-200">Email<input required type="email" value={email} onChange={(event) => setEmail(event.target.value)} className="input-field mt-1" /></label><label className="block text-sm font-medium text-slate-700 dark:text-slate-200">Password<input required type="password" value={password} onChange={(event) => setPassword(event.target.value)} className="input-field mt-1" /></label></>}<button disabled={loading} className="btn-primary w-full">{loading ? "Working..." : mode === "register" ? "Register issuer" : "Log in"}</button></form>}
